...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, cam_infos : int, spatial_lr_scale : float, opacity_on : bool, scaling : float, n_joints : int, scaling_modifier : float = 1.0, scene_type : str = "h36m"):
        ...
        scales = torch.from_numpy(np.asarray(pcd.points)).float().cuda()
        if scaling > 0.0:
            scales = torch.ones_like(scales) * scaling
            if scene_type == "h36m":
                scales[[3, 6, 12, 13, 15, 16], ...] *= scaling_modifier
            elif scene_type == "panoptic":
                scales[[8, 14, 4, 5, 10, 11], ...] *= scaling_modifier
